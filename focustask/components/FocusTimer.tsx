...
export default function FocusTimer({
  ...
  const prevLockedTaskIdRef = useRef<string | null | undefined>(undefined);
  useEffect(() => {
    if (
      prevLockedTaskIdRef.current !== undefined &&
      prevLockedTaskIdRef.current !== lockedTaskId
    ) {
      reset();
    }
    prevLockedTaskIdRef.current = lockedTaskId;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Mode tabs */}
      <div className="mx-4 mb-5 grid grid-cols-3 border border-white/8 rounded-lg overflow-hidden flex-shrink-0">
        {MODES.map((m) => (
          <button
            key={m.label}
          ...
          >
            <span
              className={
                displayMode === m.label ? "text-violet-400" : "text-white/30"
              }
            >
              {m.icon}
            </span>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-4">
          ...
          {isEditingTime ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-24 text-center bg-white/10 border border-white/20 rounded-lg text-[40px] font-mono font-bold text-white outline-none focus:border-violet-500"
                autoFocus
              />
              <span className="text-white/40 text-xl font-bold">m</span>
            </div>
          ) : (
            <span
              onClick={() => {
              ...
              className={`font-mono font-bold text-[52px] tracking-widest text-white leading-none ${!displayIsRunning && "cursor-pointer hover:text-violet-200 transition-colors"}`}
              style={{
                textShadow: displayIsRunning
        ...
        {lockedTaskTitle && (
          <p className="text-[11px] text-violet-400/80 text-center px-4">
            Working on:{" "}
            <span className="text-violet-300">{lockedTaskTitle}</span>
          </p>
        )}
        ...
        {justCompleted && (
          <div className="px-4 py-2 bg-emerald-500/15 border border-emerald-500/25 rounded-lg">
            <p className="text-[12px] text-emerald-400 font-medium text-center">
              ✓ Session complete!
            </p>
          </div>
        )}
