...
export default function TaskManagement({ onTasksChange }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !dueDate) { setFormError('Fill in all fields'); return; }
    setFormError('');
    setSubmitting(true);
    try {
      await addTask(title.trim(), priority, estimatedHours, dueDate);
      setTitle(''); setPriority('Medium'); setEstimatedHours(1); setDueDate('');
      setShowForm(false);
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3">
        <div>
          <h2 className="text-[15px] font-semibold text-white">Task Management</h2>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: '10px',
            color: showForm ? 'rgba(255,255,255,0.45)' : 'rgba(255,255,255,0.75)',
            outline: 'none',
          }}
          className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium whitespace-nowrap hover:opacity-80 transition-opacity"
        >
          {showForm ? '✕' : '+'} Add Task
        </button>
      </div>
      ...
      <div style={{
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="rgba(255,255,255,0.3)" strokeWidth="1.8" className="flex-shrink-0 ml-1">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <div className="flex-1 flex items-center gap-2.5 px-3 py-2.5" style={fieldStyle}>
              <input
                id="task-due-date"
                type="date"
                value={dueDate}
                ...
                style={{ color: dueDate ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.28)' }}
              />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-1 gap-2">
              {PRIORITIES.map((p) => (
                <button
                  ...
                  style={{
                    background: priority === p ? 'rgba(255,255,255,0.12)' : 'transparent',
                    color: priority === p ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.32)',
                    border: priority === p ? '1px solid rgba(255,255,255,0.2)' : '1px solid rgba(255,255,255,0.08)',
                    borderRadius: '8px',
                    outline: 'none',
                  }}
                  className="flex-1 py-1.5 text-[12px] font-medium transition-all duration-150 hover:opacity-80"
                >
                  {p}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2">
              <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="rgba(255,255,255,0.3)" strokeWidth="1.8">
              ...
              >
                <select
                  ...
                  style={{ background: 'transparent', color: 'rgba(255,255,255,0.7)', outline: 'none', cursor: 'pointer' }}
          ...
          {formError && (
            <p className="text-[11px] px-0.5" style={{ color: '#f87171' }}>{formError}</p>
          )}
        ...
        {(['All', 'Active', 'Completed'] as FilterTab[]).map(tab => (
          ...
          >
            ...
            {tab !== 'All'
              ...
              : counts.All > 0 && <span className="ml-1 opacity-50">({counts.All})</span>
            }
          </button>
        ))}
      </div>
